...
public class CarService {
    ...
    public List<Car> getNewCars(List<Car> cars) {
        return cars.stream().filter(car -> car.getCondition().getText().equals("NEW")).toList();
    }
    ...
    public List<CarInfo> mapToCarInfo(List<Car> cars) {
        return cars.stream().map(car -> new CarInfo(car.getName(), car.getAge(), car.getOwners().size()))
    ...
    public List<Car> getTwoBrokenCar(List<Car> cars) {
        return cars.stream().filter(car -> car.getCondition().getText().equals("BROKEN")).limit(2).toList();
    }
    ...
    public List<Car> getSortedCarsByAge(List<Car> cars) {
        return cars.stream().sorted(Comparator.comparing(Car::getAge)).toList();
    }
    ...
    public double getAvgCarsAge(List<Car> cars) {
        return cars.stream().mapToDouble(Car::getAge).average().orElse(0.0);
    }
